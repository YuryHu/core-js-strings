...
function removeFirstOccurrences(str, value) {
  const indOf1st = str.indexOf(value);
  if (indOf1st >= 0) {
    const indAfter = indOf1st + value.length;
    const before = str.slice(0, indOf1st);
    const after = str.slice(indAfter);
...
function removeLastOccurrences(str, value) {
  const indOfLast = str.lastIndexOf(value);
  if (indOfLast >= 0) {
    const indAfter = indOfLast + value.length;
    const before = str.slice(0, indOfLast);
    const after = str.slice(indAfter);
    return before.concat(after);
  }
  return str;
...
function sumOfCodes(str) {
  if (str && str.length) {
    return str.split('').reduce((total, el) => total + el.charCodeAt(0), 0);
  }
  return 0;
}
...
function formatTime(minutes, seconds) {
  const minStr = ''.concat(minutes).padStart(2, '0');
  const secStr = ''.concat(seconds).padStart(2, '0');
...
function countVowels(str) {
  ...
  return str.length - str.replaceAll(regexp, '').length;
}
...
function isPalindrome(str) {
  const newStr = str.replaceAll(/[!.,? ]/gi, '').toLowerCase();
...
function reverseWords(str) {
  const rev = (str1) => str1.split('').reverse().join('');
  return str.split(' ').map(rev).join(' ');
}
...
function invertCase(str) {
  const toggleCase = (lt) => {
    return lt === lt.toLowerCase() ? lt.toUpperCase() : lt.toLowerCase();
  };
  return str.split('').map(toggleCase).join('');
}
...
function encodeToRot13(str) {
  ...
  const getNewInd = (index) => (index + 13) % 26;
  const replace = (letter) => {
    const upInd = upper.indexOf(letter);
    const lowInd = lower.indexOf(letter);
    if (upInd >= 0) {
      return upper[getNewInd(upInd)];
    }
    if (lowInd >= 0) {
      return lower[getNewInd(lowInd)];
    }
    return letter;
  };
  return str.split('').map(replace).join('');
}
...
function getCardId(value) {
  ...
  const suits = ['♣', '♦', '♥', '♠'];
  const rate = value[0] === '1' ? '10' : value[0];
  const suite = value[0] === '1' ? value[2] : value[1];
  return rates.indexOf(rate) + 13 * suits.indexOf(suite);
}
